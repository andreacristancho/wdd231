...
async function jsonFetch(url) {
    const response = await fetch(url);  // Se espera la respuesta de la URL
    const data = await response.json(); // Se convierte la respuesta en JSON
    return data.prophets;               // Devuelve los datos de los profetas
}

const displayProphets = (prophets) => {
	const cards = document.querySelector("div.cards");
	cards.innerHTML = ""; // Limpia el contenido previo
